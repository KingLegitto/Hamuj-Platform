...
const ImageViewer: FC<ImageViewerProps> = ({
  ...
  return (
    <div ref={scrollBox} onScroll={checkScrollBox} className="fixed top-0 flex w-screen h-dvh z-[99] overflow-scroll snap-mandatory snap-x bg-[#0000007c]">
      <button
        className="fixed cursor-pointer rounded-lg top-1/2 -translate-y-1/2 left-0 z-30 px-2 py-5 lg:p-3 bg-[#00000040] backdrop-blur-sm group"
        ref={left}
        onClick={() => {
      ...
      <button
        className="fixed cursor-pointer rounded-lg top-1/2 -translate-y-1/2 right-0 z-30 px-2 py-5 lg:p-3 rotate-180 bg-[#00000040] backdrop-blur-sm group"
        ref={right}
        onClick={() => {
          handleScrolling(activeImage + 1, "smooth");
        }}
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 text-white z-40 backdrop-blur-sm font-medium px-8 py-3 bg-[#00000040] rounded-2xl">
        <span className="text-sm md:text-base">{activeImage+1}/{images.length}</span>
      </div>
    </div>
  );
};
